        ...
        {course.badges?.length > 0 && (
          <div className="absolute top-4 right-4 flex gap-2 font-bold">
            {course.badges.map((badge: string) => (
              <span
                key={badge}
                className={`text-[10px] uppercase px-2.5 py-1 rounded-lg shadow-sm font-black tracking-wider ${
                  badge === 'New' ? 'bg-blue-600 text-white' : 'bg-[#C9A961] text-white'
                }`}
              >
                {badge}
              </span>
      ...
      <section className="bg-[#2C5F2D] py-20 border-b border-[#234F24] text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 relative z-10">
            ...
            <div className="flex flex-col md:flex-row gap-5 max-w-3xl">
                <div className="relative flex-1 group md:mr-8">
                    <input 
                        type="text" 
                        placeholder="Search for courses, seerah, aqeedah..." 
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full px-12 py-7 rounded-2xl border border-white/10 bg-white text-gray-900 focus:ring-4 focus:ring-[#C9A961]/20 focus:border-[#C9A961] outline-none transition-all shadow-xl text-lg font-medium"
                    />
                </div>
                <Button className="md:ml-2 bg-[#C9A961] hover:bg-[#B89751] text-white py-7 px-10 h-auto font-black rounded-2xl text-lg shadow-xl shadow-black/10 hover:-translate-y-1 transition-all">
                    Search
                </Button>
            </div>
        </div>
            ...
            <div className="flex flex-wrap items-center gap-5 mb-12">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <button
                      ...
                      className={`px-6 py-2.5 rounded-full text-[14px] font-bold transition-all duration-300 border ${
